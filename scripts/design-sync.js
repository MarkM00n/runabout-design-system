...
// ---------------------------------------------------------------------------
// 1a-2. Compiled-class existence — catches a class name that compiles (no
// TypeScript/build error) but produces zero CSS, because Tailwind silently
// drops any candidate that doesn't match a real utility + theme value. Every
// other check in this file works from source text alone, which is exactly
// why this exact bug shipped once already: Card used `text-inverse`, which
// reads as a plausible reference to the `--color-text-inverse` token, but
// Tailwind v4 actually generates that token's color utility as
// `text-text-inverse` (property + full token name — see Button's secondary
// variant for the working example). `text-inverse` matched no generated
// rule at all, so the text silently fell back to an inherited color instead
// of erroring — invisible to every check above, only caught by rendering a
// live instance and reading getComputedStyle(). This check gets the same
// answer without a browser: it reads the CSS Tailwind's own build actually
// produced (from `npm run build-storybook`'s output, already regenerated by
// Step 4 below) and confirms every "looks like a token utility" class found
// in a component's source has a matching real rule in it.
//
// Deliberately scoped to *simple* token-style classes — a utility prefix,
// one dash, a lowercase-and-dash suffix, with any number of `variant:`
// prefixes chained in front (`hover:`, `disabled:placeholder:`, etc.).
// Arbitrary-value classes (`h-[200px]`, `bg-[#fff]/50`) are excluded
// entirely: Tailwind's escaping for bracket/hash/slash/percent characters
// is more involved than this check replicates, and those values aren't
// token references in the first place, so there's nothing token-shaped to
// verify. A class using a non-arbitrary opacity modifier on a named token
// (`bg-surface-feature/50`) would also be excluded the same way (matches
// stop at the `/`) — not currently used anywhere in this codebase, but
// worth knowing if one shows up and needs checking too. An arbitrary
// *variant* in front (`[label:has(:disabled)_&]:text-text-muted`, used by
// Checkbox) is excluded the same way, by dropping any match immediately
// preceded by `]:` on the line — the base utility after it isn't a
// standalone class Tailwind generates its own bare rule for, only reachable
// through that whole bracketed selector, which needs the same fuller
// escaping this check isn't attempting to replicate.
const ALL_UTILITY_PREFIXES = [...new Set(Object.values(TOKEN_UTILITY_PREFIXES).flat())].sort(
  (a, b) => b.length - a.length,
);
const UTILITY_PREFIX_ALTERNATION = ALL_UTILITY_PREFIXES.map((p) =>
  p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
).join('|');
// Full class token, including every chained `variant:` prefix (zero or
// more) — captured as one string via match[0], not sub-groups, since the
// compiled-CSS lookup needs the exact original text to escape correctly.
const CLASS_CANDIDATE_RE = new RegExp(
  `\\b(?:[a-z][a-z-]*:)*(?:${UTILITY_PREFIX_ALTERNATION})-[a-z][a-z0-9-]*\\b`,
  'g',
);

// Recursively concatenates every .css file under storybook-static/ into one
// string — Tailwind's actual compiled output, split across an iframe bundle
// plus a handful of per-page/per-component chunks with content-hashed
// filenames that change on every build, so this reads whatever's there
// rather than hardcoding a name. Returns null if the directory doesn't
// exist (build-storybook hasn't run, or failed before producing output) —
// callers must treat that as "can't check this run", not "check passed".
export function readCompiledCss(dir = STORYBOOK_STATIC_DIR) {
  ...
  const walk = (current) => {
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      else if (entry.name.endsWith('.css')) css += readFileSync(full, 'utf8');
    }
  };
  walk(dir);
  return css || null;
...
// Tailwind escapes every character outside [a-zA-Z0-9_-] in a class name
// when emitting its CSS selector — the only such character our candidates
// can contain is `:` (from chained variant prefixes), escaped as `\:`. A
// real selector always starts the class name with exactly one literal `.`
// immediately followed by this escaped text (with the variant's actual
// pseudo-class/pseudo-element, e.g. `:hover`, appended unescaped right
// after) — so a plain substring search for `.` + escaped-candidate,
// confirming the next character isn't itself an identifier character (which
// would mean this candidate is only a prefix of some longer, unrelated
// class), is enough to tell "real rule" from "nothing generated" without
// needing to reconstruct the full selector Tailwind wrote.
function classExistsInCompiledCss(candidate, compiledCss) {
  const needle = '.' + candidate.replace(/:/g, '\\:');
  let from = 0;
  while (true) {
    const idx = compiledCss.indexOf(needle, from);
    if (idx === -1) return false;
    const nextChar = compiledCss[idx + needle.length];
    if (nextChar === undefined || !/[a-zA-Z0-9_-]/.test(nextChar)) return true;
    from = idx + 1;
  }
}
...
export function checkCompiledClassesExist(name, source, compiledCss) {
  ...
  codeOnly.split('\n').forEach((line, i) => {
    CLASS_CANDIDATE_RE.lastIndex = 0;
    let match;
    while ((match = CLASS_CANDIDATE_RE.exec(line))) {
      if (line.slice(0, match.index).endsWith(']:')) continue; // target of an arbitrary variant, not standalone
      const candidate = match[0];
      const key = `${candidate}:${i + 1}`;
      if (seen.has(key)) continue;
      seen.add(key);
      if (!classExistsInCompiledCss(candidate, compiledCss)) {
        issues.push({
          level: 'fail',
          file,
          line: i + 1,
...
function run() {
  ...
  // First pass: everything that doesn't depend on Tailwind's actual compiled
  // output — base token-text checks, accessibility, Storybook/doc coverage,
  // stub generation. Finalizing each component's report (which now also
  // needs the compiled-class check below) is deferred to a second pass after
  // Step 4's build, since that's the earliest point real compiled CSS exists
  // to check against.
  ...
  // run any earlier than this: it's the first point actual compiled output
  // exists to check candidate classes against. If the build failed, there's
  // no trustworthy output to check against, so this check is skipped for the
  ...
  for (const pending of pendingComponents) {
    const { name, dir, componentSource, baseTokenIssues, a11yIssues, storybookIssues, tokensUsed, docIssues, generated } =
      pending;

    const compiledClassIssues = checkCompiledClassesExist(name, componentSource, compiledCss);
    const tokenIssues = [...baseTokenIssues, ...compiledClassIssues];
