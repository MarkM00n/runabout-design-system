...
export const SignUpFormCard = ({ startTouched = false }: VinesAndVinylSignUpProps) => {
  ...
  // Separate from `touched`: tracks whether the field's value has actually
  // been changed at least once, vs. merely visited-and-left. Blur alone
  // used to be enough to mark a field touched, which meant tabbing through
  // the whole empty form — never typing anything — showed all three
  // required-field errors before the person had done anything wrong. Blur
  // should only trigger validation for a field the person actually edited;
  // an untouched empty field they just tabbed past isn't an error yet.
  const [dirty, setDirty] = useState<Record<RequiredField, boolean>>({
    name: false,
    email: false,
    ticketType: false,
  });
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (!isValid) {
      // Validate everything on submit, regardless of which fields were
      // actually edited — an empty required field left completely alone
      // is still a reason submit can't proceed, and the person needs to
      // see that, not just the fields they happened to touch first.
      setTouched({ name: true, email: true, ticketType: true });
      return;
    }
    setSubmitted(true);
  };
    ...
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-04">
      ...
      <div className="flex flex-col gap-01">
        ...
        <Select
          ...
          onChange={(e) => {
            setValues((v) => ({ ...v, ticketType: e.target.value as TicketType }));
            setDirty((d) => ({ ...d, ticketType: true }));
          }}
